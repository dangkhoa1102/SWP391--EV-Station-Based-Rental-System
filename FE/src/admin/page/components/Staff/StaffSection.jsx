...
export default function StaffSection({ 
  ...
  // IMPORTANT: users array has priority because it contains station assignments loaded at page initialization
  const allStaff = useMemo(() => {
    const staffMap = new Map();
    
    // Add staff from staffByStation first (lower priority)
    ...
    // Add/override with staff from users array (HIGHER PRIORITY - has station assignments)
    users.forEach(user => {
      const role = (user.role || user.Role || user.userRole || '').toLowerCase();
      if (role === 'station staff' || role === 'stationstaff' || role.includes('staff')) {
        const id = user.id || user.Id || user.userId;
        if (id) {
          // Override or add - users data has priority because it has stationId populated
