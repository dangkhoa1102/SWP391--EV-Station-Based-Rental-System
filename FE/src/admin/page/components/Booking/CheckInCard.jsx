...
import { performAutoCheckIn, getCheckInInfoAutomatic } from '../../../services/adminCheckInAutomatic';

export default function CheckInCard({ booking, onClose, onSuccess }) {
  ...
  async function handleSubmit(e){
    ...
    try {
      ...
      if (typeof onSuccess === 'function') onSuccess(booking.id, result)
      onClose?.()
    } catch (e) {
  ...
  return (
    <div className="modal-overlay" style={{display:'flex'}}>
      <div className="modal-content" style={{width:'min(720px,95vw)', maxHeight:'90vh', overflow:'auto'}}>
        ...
        {error && <div style={{background:'#ffecec', color:'#b00020', padding:'8px 12px', borderRadius:6, marginBottom:12, whiteSpace:'pre-wrap', fontSize:'0.95em'}}>{error}</div>}
