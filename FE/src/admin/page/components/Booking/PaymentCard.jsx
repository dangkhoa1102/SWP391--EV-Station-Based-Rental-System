    ...
    async function load() {
      ...
      try {
        ...
        if (!booking.totalAmount && !booking.TotalAmount) {
          try {
            const b = await AdminAPI.getBookingById(booking.id)
            if (!mounted) return
        ...
        try {
          const res = await AdminAPI.post('/Payment/create', {
        ...
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: '1 1 320px', minWidth: 280 }}>
            <div style={{ display: 'flex', gap: 12 }}>
              <img
                src={booking.img || expandedBooking?.carImageUrl || expandedBooking?.car?.imageUrl || PLACEHOLDER.small('Car')}
                alt="car"
