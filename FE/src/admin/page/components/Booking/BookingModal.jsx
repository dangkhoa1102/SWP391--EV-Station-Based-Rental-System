...
export default function BookingModal({ booking, onClose, onProceed, onCancel, onStatusUpdated }) {
  ...
  const canProceed = (booking?.status === 'booked' || booking?.status === 'contracted') ? (verifiedFront && verifiedBack) : false
  const onCancelClick = () => {
    if (!booking) return;
      ...
      <div className="modal-content" style={{display:'flex', flexDirection:'column', width:'min(920px,95vw)', maxHeight:'90vh', overflow:'auto', margin:'0 auto'}}>
        ...
        <div style={{display:'flex', gap:24, flexWrap:'wrap'}}>
          ...
          <div style={{flex:'1 1 380px', display:'flex', flexDirection:'column', gap:14}}>
            ...
            {/* CCCD Front/Back Images with verification checkboxes - only when status is Booked or Contracted */}
            {(booking.status === 'booked' || booking.status === 'contracted') && (
            ...
            {/* Driver License (GPLX) Images with verification checkboxes - only when status is Booked or Contracted */}
            {(booking.status === 'booked' || booking.status === 'contracted') && (
              <div className="bg-gray-100" style={{padding:14, borderRadius:10}}>
