  ...
  createCar: async (payload) => {
    ...
    for (const url of attempts) {
      try {
        ...
        const res = await apiClient.post('/Cars/Create', formData)
        const body = res.data
        return body && typeof body === 'object' && 'data' in body ? body.data : body
      } catch (e) {
        const code = e?.response?.status
...
API.getStaffByStation = async (stationId) => {
  ...
  for (const a of attempts) {
    try {
      ...
      const body = res?.data
      console.log(`📋 [Admin] getStaffByStation response for ${stationId}:`, body)
      
      // Try multiple response formats
      if (Array.isArray(body)) {
        console.log(`✅ [Admin] Found ${body.length} staff members (direct array)`)
        return body
      }
      if (body?.data && Array.isArray(body.data)) {
        console.log(`✅ [Admin] Found ${body.data.length} staff members (body.data)`)
        return body.data
      }
      if (body?.items && Array.isArray(body.items)) {
        console.log(`✅ [Admin] Found ${body.items.length} staff members (body.items)`)
        return body.items
      }
      
      console.warn(`⚠️ [Admin] Unexpected response format for station ${stationId}:`, typeof body, Object.keys(body || {}))
    } catch (e) {
      const code = e?.response?.status
      console.warn(`⚠️ [Admin] Error getting staff for station ${stationId}:`, code, e.message)
      if (code && code !== 404 && code !== 405) throw e
    }
  }
  console.warn(`⚠️ [Admin] No staff found for station ${stationId}`)
  return []
}

/**
 * Get station for a specific staff member (reverse of getStaffByStation)
 * Since the API only provides /Admin/Staff/By-Station/{stationId} (get staff BY station),
...
API.getStationByStaff = async (staffId) => {
  ...
  console.log('🔍 [Admin] Looking for station assigned to staff:', staffId)
  ...
  for (const url of attempts) {
    try {
      ...
      if (result) {
        console.log('✅ [Admin] Found station via direct endpoint:', result)
        return result
      }
    } catch (e) {
      const code = e?.response?.status
  ...
  try {
    ...
    if (profile && (profile.stationId || profile.StationId)) {
      console.log('✅ [Admin] Found station via user profile:', profile.stationId || profile.StationId)
      return {
    ...
    console.warn('[Admin] User profile fallback failed:', e.message)
  }
  
  // Fallback: Search all stations and find which one has this staff
  // This is the most reliable method since the API only provides getStaffByStation
  try {
    console.log('🔍 [Admin] Searching all stations for staff assignment...')
    const stations = await API.getAllStations()
    
    if (!stations || stations.length === 0) {
      console.error('❌ [Admin] No stations returned from getAllStations()')
      return null
    }
    
    console.log(`📍 [Admin] Loaded ${stations.length} stations to search`)
    console.log('📍 [Admin] Station IDs:', stations.map(s => s.id || s.Id))
    
    let checkedCount = 0
    for (const station of stations) {
      const stationId = station.id || station.Id
      if (!stationId) {
        console.warn('   Skipping station with no ID:', station)
        continue
      }
      
      checkedCount++
      console.log(`   [${checkedCount}/${stations.length}] Checking station ${stationId} (${station.name || station.Name || 'Unnamed'})`)
      
      try {
        const staff = await API.getStaffByStation(stationId)
        ...
        const found = staff.find(s => {
          ...
          const matches = sUserId === staffId
          if (staff.length <= 5 || matches) { // Only log if few staff or if found match
            console.log(`         Comparing ${sUserId} === ${staffId}? ${matches}`)
          }
          return matches
        })
        
        if (found) {
          console.log(`✅ [Admin] MATCH FOUND! Staff ${staffId} assigned to station ${stationId}`)
          console.log(`   Staff details:`, found)
          return {
            stationId: stationId,
            stationName: station.name || station.Name || `Station ${stationId}`,
            userId: staffId,
            userRole: found.userRole || found.role || found.Role,
            id: stationId,
            Id: stationId,
            name: station.name || station.Name || `Station ${stationId}`,
            Name: station.name || station.Name || `Station ${stationId}`,
            ...station
          }
        } else {
          console.log(`      → Staff ${staffId} NOT in this station, continuing...`)
        }
      } catch (e) {
    ...
    console.warn(`⚠️ [Admin] Searched all ${checkedCount} stations, staff ${staffId} not found in any`)
  } catch (e) {
    // Final fallback failed
    console.error('❌ [Admin] All station search fallbacks failed:', e)
  }
  
  console.warn('⚠️ [Admin] No station assignment found for staff:', staffId)
  return null
}
