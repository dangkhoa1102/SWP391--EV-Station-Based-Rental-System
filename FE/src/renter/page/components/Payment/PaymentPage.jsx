...
export default function PaymentPage(){
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (carId) {
        carApi.getCarById(carId).then(carData => setCar(carData))
      }
    } catch (e) {
      console.error(e)
    }
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      ...
      const bookingResponse = await bookingApi.createBooking(bookingData, userId)
      ...
      const contractResponse = await contractApi.createRentalContract(newBookingId, userId, contractDto)
      // ContractId could be in contractResponse.id, contractResponse.contractId, or the whole object could be the contract
      const contractId = contractResponse?.id || contractResponse?.contractId || contractResponse
      console.log('✅ Contract created with ID:', contractId)
      console.log('📄 Full contract response:', contractResponse)
      ...
      try {
        ...
        if (userEmail && contractId) {
          ...
          const emailResponse = await contractApi.sendContractEmail(contractId, userEmail)
