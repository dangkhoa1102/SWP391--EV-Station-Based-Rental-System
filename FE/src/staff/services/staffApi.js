...
const apiClient = axios.create({
  ...
  timeout: 30000 // Increased to 30 seconds to handle slow endpoints
})
...
apiClient.interceptors.response.use(
  ...
  e => {
    try {
      ...
      // Don't log 404/400 errors - they're expected when trying multiple endpoint variants
      // NOTE: You will still see these in the browser's Network tab, which is normal browser behavior
      // and cannot be suppressed. This interceptor only prevents console.error() logging.
...
const API = {
  ...
  getAllCars: async (pageNumber = 1, pageSize = 100) => {
    ...
    for (const url of attempts) {
      try {
        console.log(`🚗 Trying to fetch cars from: ${url}`)
        const res = await apiClient.get(url, { params: { pageNumber, pageSize } })
        console.log('✅ Cars response from', url, ':', res.data)
        const responseData = res.data
        ...
        console.warn('⚠️ No cars found in response from', url)
        return []
      } catch (e) {
        lastErr = e
        const code = e?.response?.status
        // Only continue trying other endpoints on 404/405, otherwise throw
        if (code && code !== 404 && code !== 405) {
          console.error('❌ Error fetching cars from', url, ':', e.response?.data || e.message)
          return []
        }
      }
    }
    
    console.error('❌ All car endpoints failed. Last error:', lastErr?.response?.data || lastErr?.message)
    return []
  },
  ...
  getBookingById: async (bookingId) => {
    ...
    // Return null instead of throwing - let the caller handle missing bookings
  ...
  getVehicleModelFromBooking: async (bookingId) => {
    ...
    // If booking doesn't exist, return null instead of throwing
    if (!booking) {
      console.warn(`⚠️ Cannot get vehicle model - booking ${bookingId} not found`)
      return null
    }
  ...
  getUserNameByBookingId: async (bookingId) => {
    ...
    try {
      ...
      // If booking doesn't exist, throw early
...
API.checkInWithContract = async (payload) => {
  // Required: bookingId, staffId
  // Optional: checkInNotes, checkInPhotoUrl
  ...
  const requestPayload = {
    ...
    checkInNotes: payload.checkInNotes || 'Car checked in',
    checkInPhotoUrl: payload.checkInPhotoUrl || null,
    staffSignature: payload.staffSignature || 'placeholder',
    customerSignature: payload.customerSignature || 'placeholder',
  }
  
  console.log('🔍 Check-in payload:', requestPayload);
  ...
  for (const url of attempts) {
    try {
      console.log(`📍 Attempting check-in at: ${url}`);
      const res = await apiClient.post(url, requestPayload)
      const body = res?.data
      console.log('✅ Check-in response:', body);
      ...
      console.warn(`⚠️ Check-in attempt failed at ${url}:`, e?.response?.status, e?.response?.data);
 ...
 */
API.uploadImage = async (imageFile) => {
  ...
  for (const url of attempts) {
    try {
      ...
      if (typeof body === 'string' && (body.startsWith('http') || body.startsWith('/'))) {
        return body
      }
      if (body?.data?.url) return body.data.url
      if (body?.data?.imageUrl) return body.data.imageUrl
  ...
  throw lastErr || new Error('Image upload endpoint not found')
}

/**
 * Get the station assigned to a specific staff member
 * Since the API only provides /Stations/By-Staff/{stationId} (get staff BY station),
 * we need to reverse the lookup: iterate through all stations and find which one
 * has this staff member assigned.
 * @param {string} staffId - Staff user ID (userId, not staffId entity)
 * @returns {Promise<Object>} Station object with id and name
 */
API.getStationByStaff = async (staffId) => {
  if (!staffId) throw new Error('staffId is required')
  
  console.log('🔍 Looking for station assigned to staff:', staffId)
  
  try {
    // Step 1: Get all stations
    const stations = await API.getAllStations(1, 100)
    ...
    // Step 2: For each station, check if this staff is assigned to it
    for (const station of stations) {
      const stationId = station.id || station.Id || station.stationId || station.StationId
      if (!stationId) continue
      
      console.log(`🔍 Checking station ${stationId} for staff ${staffId}`)
      
      try {
        // Try to get staff members for this station
        const staffResponse = await apiClient.get(`/Admin/Staff/By-Station/${encodeURIComponent(stationId)}`)
        ...
        if (Array.isArray(responseData)) {
          ...
          staffList = responseData.items
        }
        
        console.log(`   Found ${staffList.length} staff members at station ${stationId}`)
        
        if (staffList.length > 0) {
          console.log(`   Staff list sample:`, staffList[0])
        }
        
        // Check if our staff is in this list
        const foundStaff = staffList.find(s => {
          const sUserId = s.userId || s.UserId || s.id || s.Id
          console.log(`      Comparing staff userId: ${sUserId} with target: ${staffId}`)
          return sUserId === staffId
        })
        
        if (foundStaff) {
          console.log(`✅ Found staff ${staffId} assigned to station ${stationId}`)
          console.log(`   Staff details:`, foundStaff)
          return {
            id: stationId,
            Id: stationId,
            stationId: stationId,
            name: station.name || station.Name || station.stationName || `Station ${stationId}`,
            Name: station.name || station.Name || station.stationName || `Station ${stationId}`,
            ...station
          }
        }
      } catch (e) {
        // Station might not have any staff, continue to next
        console.warn(`   Error checking station ${stationId}:`, e.message)
        continue
      }
    }
    
    throw new Error('No station assignment found for this staff member')
  } catch (e) {
    console.error('❌ Error finding station for staff:', e)
    throw e
  }
}

/**
 * Get the current logged-in staff member's station (for authorization)
 * Uses the dedicated /Auth/my-station endpoint that returns station info
 * based on the authenticated user's JWT token.
 * @returns {Promise<Object>} Staff and station details
 */
export const getMyStation = async () => {
  ...
  try {
    const response = await fetch(`${SWAGGER_ROOT}/Auth/my-station`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      if (response.status === 401) {
        throw new Error('Unauthorized. Please login as staff member.');
      }
    ...
    console.log('✅ Retrieved staff station from /Auth/my-station:', data.data);
    return data.data;
  } catch (error) {
    console.error('❌ Error fetching staff station from /Auth/my-station:', error);
    throw error;
  }
};

/**
 * Check if staff is authorized for a specific station
 * @param {string} requiredStationId - The station ID to check authorization for
 * @returns {Promise<boolean>} True if authorized, false otherwise
 */
export const isStaffAuthorizedForStation = async (requiredStationId) => {
  try {
    const staffInfo = await getMyStation();
    
    if (!staffInfo.station) {
      console.warn('⚠️ Staff not assigned to any station');
      return false;
    }

    const assignedStationId = staffInfo.station.stationId || staffInfo.station.id;
    const isAuthorized = assignedStationId === requiredStationId;
    
    console.log(`🔐 Authorization check: ${isAuthorized ? 'AUTHORIZED' : 'DENIED'} for station ${requiredStationId}`);
    console.log(`   Staff assigned to: ${assignedStationId}`);
