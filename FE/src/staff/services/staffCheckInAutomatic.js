...
export const getCheckInInfoAutomatic = async (bookingId) => {
  ...
  try {
    ...
    // 4️⃣ Get Staff Information (current staff ID from token)
    const staffUserData = JSON.parse(atob(staffToken.split('.')[1]));
    const staffId = staffUserData.nameid || staffUserData.sub;
...
/**
 * One-click check-in: Staff just provides bookingId and optional notes
