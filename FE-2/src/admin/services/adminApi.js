...
const API = {
  ...
  // ========================= Stations (Admin) =========================
  // Backend: Monolithic/Controllers/StationsController.cs
  stationsList: async (pageNumber = 1, pageSize = 100) => {
    const res = await apiClient.get('/Stations/Get-All', { params: { pageNumber, pageSize } })
    const body = res.data
  ...
  stationGetById: async (stationId) => {
    const id = encodeURIComponent(stationId)
    const res = await apiClient.get(`/Stations/Get-By-${id}`)
    const body = res.data
    return (body && typeof body === 'object' && 'data' in body) ? body.data : body
  },
  stationCreate: async (payload) => {
    // CreateStationDto expected in body (JSON)
    const res = await apiClient.post('/Stations/Create', payload)
    const body = res.data
    return (body && typeof body === 'object' && 'data' in body) ? body.data : body
  },
  stationUpdate: async (stationId, payload) => {
    const id = encodeURIComponent(stationId)
    const res = await apiClient.put(`/Stations/Update-By-${id}`, payload)
    const body = res.data
    return (body && typeof body === 'object' && 'data' in body) ? body.data : body
  },
  stationDelete: async (stationId) => {
    const id = encodeURIComponent(stationId)
    const res = await apiClient.delete(`/Stations/Delete-By-${id}`)
    const body = res.data
    return (body && typeof body === 'object' && 'data' in body) ? body.data : body
  },
  stationGetAvailableCars: async (stationId) => {
    const id = encodeURIComponent(stationId)
    const res = await apiClient.get(`/Stations/Get-Available-Cars-By-${id}`)
    const body = res.data
    return (body && typeof body === 'object' && 'data' in body) ? body.data : body
  },
  stationRecalculateSlots: async (stationId) => {
    const id = encodeURIComponent(stationId)
    const res = await apiClient.post(`/Stations/Recalculate-Slots-By-${id}`)
    const body = res.data
    return (body && typeof body === 'object' && 'data' in body) ? body.data : body
  },
  ...
  createCar: async (payload = {}) => {
    const requiredFields = ['brand','model','year','licensePlate','batteryCapacity','currentBatteryLevel','rentalPricePerHour','rentalPricePerDay','currentStationId','seats']
    for (const key of requiredFields) {
      if (payload[key] == null || payload[key] === '') {
        throw new Error(`Missing required car field: ${key}`)
      }
    }
    ...
    const buildFormData = () => {
      ...
      appendField(form, 'Brand', payload.brand)
      appendField(form, 'Model', payload.model)
      appendField(form, 'Year', payload.year)
      appendField(form, 'Color', payload.color || payload.Color)
      appendField(form, 'LicensePlate', payload.licensePlate)
      appendField(form, 'BatteryCapacity', payload.batteryCapacity)
      appendField(form, 'CurrentBatteryLevel', payload.currentBatteryLevel)
      appendField(form, 'RentalPricePerHour', payload.rentalPricePerHour)
      appendField(form, 'RentalPricePerDay', payload.rentalPricePerDay ?? payload.rentalPricePerDate)
      appendField(form, 'CurrentStationId', payload.currentStationId || payload.stationId || payload.StationId)
      appendField(form, 'Seats', payload.seats)
      if (isBrowserFile(payload.carImage)) {
        form.append('CarImage', payload.carImage)
      }
      return form
    }
    ...
    const jsonPayload = {
      ...
      CurrentBatteryLevel: payload.currentBatteryLevel,
      RentalPricePerHour: payload.rentalPricePerHour,
      RentalPricePerDay: payload.rentalPricePerDay ?? payload.rentalPricePerDate,
      CurrentStationId: payload.currentStationId || payload.stationId || payload.StationId,
      Seats: payload.seats
    }

    const attempts = [
      { url: '/Cars/Create', body: formData, isForm: true },
      { url: '/Car/Create', body: formData, isForm: true },
      { url: '/Vehicle/Create', body: formData, isForm: true },
      { url: '/car', body: jsonPayload },
      { url: '/cars', body: jsonPayload },
      { url: '/vehicle', body: jsonPayload },
      { url: '/vehicles', body: jsonPayload }
    ]
    ...
    for (const attempt of attempts) {
      try {
        ...
        const config = attempt.isForm ? { headers: { 'Content-Type': 'multipart/form-data' } } : undefined
        const res = await apiClient.post(attempt.url, attempt.body, config)
        const body = res.data
        return body && typeof body === 'object' && 'data' in body ? body.data : body
      } catch (e) {
        const code = e?.response?.status
