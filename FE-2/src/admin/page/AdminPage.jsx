...
export default function AdminPage() {
  ...
  const addVehicle = async (payload) => {
    try {
      ...
      normalized.StationId = stationId
      normalized.seats = payload.seats ?? 4
      ...
      if (payload.carImage) {
        normalized.carImage = payload.carImage
      }
        ...
        {section === 'station' && (
          <>
            <StationSection
              ...
              onReload={async () => {
                try {
                  const s = await AdminAPI.stationsList(1, 100)
                  setStations(s || [])
                } catch (e) {
                  setError(e?.message || 'Failed to reload stations')
                }
              }}
            />
          </>
