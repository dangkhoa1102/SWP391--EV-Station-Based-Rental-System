        ...
        public async Task<ResponseDto<BookingDto>> CheckOutBookingAsync(CheckOutWithPaymentDto request)
   {
       try
       {
           ...
           if (actualReturn <= expectedReturn)
           {
               booking.LateFee = 0;
               booking.ExtraAmount = 0;
               var refund = booking.DepositAmount - booking.DamageFee;
               booking.RefundAmount = refund > 0 ? refund : 0;
               booking.FinalPaymentAmount = -booking.RefundAmount;
               booking.DepositRefunded = booking.RefundAmount > 0;
               booking.ExtraAmount = refund < 0 ? -refund : 0;
               ...
               string message1;
               if (booking.ExtraAmount > 0)
                   message1 = $"Early return: Deposit not enough to cover damage. Extra payment required: {booking.ExtraAmount:C}.";
               else
                   message1 = $"Early return: Deposit refunded {booking.RefundAmount:C} after deducting damage fee.";

               return ResponseDto<BookingDto>.Success(dto, message1);
           }                        
